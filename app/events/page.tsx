...
import { eventImages, images } from "@/lib/data/images";
            ...
            <FadeIn key={event.id} delay={i * 0.1}>
              <Link href={`/events/${event.slug}`}>
                <Card className="p-8 flex flex-col md:flex-row gap-6 hover:border-primary/20 transition-all group">
                  <ContentImage
                    src={event.thumbnail ?? eventImages[event.slug] ?? images.eventWebinar}
                    alt={event.title}
                    aspect="video"
                  ...
                  <div className="flex-1">
                    ...
                    <div className="flex gap-6 mt-4 text-sm text-muted">
                      ...
                      <span className="text-primary font-medium">{event.price === 0 ? "Free" : formatPrice(event.price)}</span>
                    </div>
                  </div>
                </Card>
              </Link>
